...
Contexto de los componentes DS v2 para Claude Code. Leer antes de cualquier acción.
Para el contexto del monorepo completo, ver el `CLAUDE.md` de la raíz.
...
> **Nota de estructura:** los componentes `pds-*` **ya no viven en este proyecto**. Se extrajeron al paquete `@poli/components` (`packages/components/src/lib/`) del monorepo. La semilla los consume como dependencia (`"@poli/components": "workspace:*"`). Las convenciones de este documento aplican al desarrollo de esos componentes en su nueva ubicación.
...
Los archivos CSS generados por Style Dictionary están en `packages/tokens/src/` (fuente única de verdad, expuesta como `@poli/tokens`):
...
Se cargan en el `styles` array de cada consumidor, antes de su `styles.scss`:

- **Semilla**: `apps/semilla-front/angular.json` → `node_modules/@poli/tokens/src/*.css`
- **Storybook**: `angular.json` (proyecto `storybook-host`) → `packages/tokens/src/*.css`

Los overrides mobile están en el `styles.scss` de cada app dentro de `@media (max-width: 768px)`.
...
## Estructura relevante (monorepo)

```
packages/
  tokens/src/                        ← tokens v2, fuente única de verdad (@poli/tokens)
    primitives.css / tokens.css      ← generados por Style Dictionary
    typescale-desktop.css            ← no editar a mano
    layout-desktop.css
    component.css / component-overrides.css
  components/src/lib/
    pds-*/                           ← componentes DS v2 (aquí trabajamos)
    public-api.ts                    ← barrel de exports; todo componente nuevo se exporta aquí

apps/semilla-front/src/app/
  core/auth/                         ← autenticación MSAL (ver RESTAURAR_AUTH.md)
  layout/                            ← shell autenticado (header + sidenav)
  pages/                             ← pantallas de referencia que consumen @poli/components
```
...
- Selector: `pds-{nombre}`
- Carpeta: `packages/components/src/lib/pds-{nombre}/`
- Archivos: `pds-{nombre}.component.ts/html/scss/stories.ts`
- Export obligatorio en `packages/components/src/public-api.ts`
...
- [x] `pds-breadcrumb` — separadores con chevron, `aria-current="page"` en el último ítem
- [x] `pds-paginator` — navegación de páginas con estados disabled en extremos
...
### Fase 6 — Complejos ✅ COMPLETA

- [x] `pds-date-picker`, `pds-file-uploader`, `pds-range`
- [x] `pds-accordion`, `pds-table`, `pds-code-block`
- [x] `pds-selectable-card` — **implementado como `pds-card`** con inputs `selectable`/`selected` y `aria-pressed`. El nombre difiere del spec.
